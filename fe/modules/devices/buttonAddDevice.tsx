...
export default function ButtonAddDevice() {
  ...
  const handleAddDevice = async (e?: React.FormEvent) => {
    ...
    const emailRegex =
      /^[a-zA-Z0-9._%+-]+@(gmail\.com|mail\.ugm\.ac\.id|ugm\.ac\.id)$/;
    if (!emailRegex.test(email)) {
      setFailedData({
        show: true,
        message:
          "Email must end with @gmail.com, @mail.ugm.ac.id, or @ugm.ac.id",
      });
      return;
    }
