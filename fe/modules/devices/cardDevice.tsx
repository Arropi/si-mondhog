...
export default function CardDevice({
      ...
      try {
        ...
        if (os && os !== "all") {
          filteredData = filteredData.filter(
            (d) => d.os.toLowerCase() === os.toLowerCase(),
          );
        }
    ...
    return (
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 md:gap-6 mt-8">
        ...
        {[...Array(10)].map((_, i) => (
          <div
            key={i}
            id={`devices-loading-card-${i}`}
            className="bg-white rounded-xl p-6 shadow-sm border border-gray-100 flex flex-col gap-4 animate-pulse h-56 w-full"
          >
  ...
  if (devices.length === 0) {
    return (
      <div
        id="devices-empty-state"
        className="flex flex-col items-center justify-center mt-16 mb-24"
      >
        <Image
          id="devices-empty-image"
        ...
        />
        <p className="text-black mt-6 font-light">
          Looking everywhere, but it’s not here...
        </p>
      </div>
    );
  }
