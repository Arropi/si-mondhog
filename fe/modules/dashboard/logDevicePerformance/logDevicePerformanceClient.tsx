...
export default function LogsGeneralClient({
  initialData,
}: {
  initialData: PerformanceLogFormatted[];
}) {
  const [logs] = useState(initialData);

  return (
    <div
      id="dashboard-performance-logs-card"
      className="bg-white rounded-[24px] p-8 shadow-[0_2px_20px_rgba(0,0,0,0.02)] border border-gray-100"
    >
      <div
        id="dashboard-performance-logs-header"
        className="flex justify-between items-center mb-6"
      >
        <h2
          id="dashboard-performance-logs-title"
          className="text-[15px] font-bold text-[#FF0B5B] tracking-wide"
        >
          Logs Device Performance
        </h2>
        <div id="dashboard-performance-logs-download" className="relative">
          <ButtonDownloadPerformance />
        </div>
      </div>

      <div className="w-full overflow-x-auto">
        <div id="performance-logs-table" className="min-w-[700px]">
          {/* Table Header */}
          <div id="performance-logs-table-header" className="grid grid-cols-7 text-[12px] font-bold text-gray-800 border-b border-gray-100 pb-4 pt-2">
            <div>Date</div>
            <div>Name</div>
            <div>OS</div>
            <div>CPU</div>
          ...
          {/* Table Body */}
          <div id="performance-logs-table-body" className="flex flex-col">
            {logs.map((log, idx) => (
              <div
                key={idx}
                id={`performance-log-row-${idx}`}
      ...
      <div className="mt-4 grid grid-cols-5">
        <p id="performance-logs-note" className="col-start-5 text-xs text-gray-500">
