...
export default function DatePickerClient({
  ...
  return (
    <div
      id="dashboard-date-picker-days-list"
      className="flex justify-between gap-3 overflow-x-auto pb-2 -mx-1 px-1 custom-scrollbar"
    >
      {dates.map((item, idx) => (
        <div
          key={idx}
          id={`dashboard-date-option-${item.fullDate}`}
        ...
        >
          <span
            id={`dashboard-date-option-day-${item.fullDate}`}
            className={cn(
              "text-[10px] font-bold tracking-wide uppercase mb-1",
              activeDate === item.fullDate ? "text-purple-200" : "",
            )}
          >
            {item.day}
          </span>
          <span
            id={`dashboard-date-option-date-${item.fullDate}`}
            className={cn(
              "text-2xl font-black",
              activeDate === item.fullDate ? "text-white" : "text-gray-700",
            )}
          >
            {item.date}
          </span>
