...
export default function LogDeviceActivityClient({ deviceId, initialData }: LogDeviceActivityProps) {
    ...
    return (
        <div id="device-activity-log-card" className="bg-white rounded-2xl p-6 shadow-[0_2px_10px_rgba(0,0,0,0.04)] border border-gray-50 flex flex-col mt-2">
            <h2 id="device-activity-log-title" className="text-[13px] font-extrabold text-purple-700 tracking-wide mb-5">
                Log Device Activity
            </h2>

            <div className="flex-1">
                <div id="device-activity-log-header" className="grid grid-cols-4 text-[11px] font-bold text-gray-400 uppercase tracking-widest mb-4 border-b border-gray-50 pb-3">
                    <div>CPU</div>
                    <div>RAM</div>
                    <div>Harddisk</div>
                    <div>Timestamp</div>
                ...
                <div id="device-activity-log-body">
                    {logs.length > 0 ? (
                        logs.map((log: DeviceActivityLog, i: number) => (
                            <div
                                key={i}
                                id={`device-activity-log-row-${i}`}
                        ...
                        <div id="device-activity-log-empty" className="text-center text-gray-400 text-xs py-4">
