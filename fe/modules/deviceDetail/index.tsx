...
export default async function DetailDevice({
  ...
  const isEmptyPending =
    status === "Pending" || (hasNoMetrics && status !== "Offline");
  const isOfflineNoData = status === "Offline" && hasNoMetrics;

  return (
    <div
      id="device-detail-page"
      className="w-full min-h-screen bg--background pb-10 animate-fade-in"
    >
      <div
        id="device-detail-topbar"
        className="flex flex-col md:flex-row justify-between items-start md:items-center py-6 px-8"
      >
        <div
          id="device-detail-breadcrumb"
          className="text-sm font-medium text-gray-500 mb-4 md:mb-0"
        >
          <Link
            id="device-detail-breadcrumb-link"
            href="/devices"
        ...
        <div id="device-detail-actions" className="flex items-center gap-3">
      ...
      <div
        id="device-detail-layout"
        className="px-8 grid grid-cols-1 lg:grid-cols-12 gap-6"
      >
        {/* kiri */}
        <div
          id="device-detail-left-column"
          ...
          {isEmptyPending ? (
            ...
            <>
              <PerformanceSummaryClient
                ...
                initialData={{
                  highestCpuUsage: highestStats?.length
                    ? Math.round(highestStats[0].cpu)
                    : 0,
                  cpuChange:
                    highestStats?.length && logs?.length
                      ? Math.max(
                          0,
                          Math.round(highestStats[0].cpu - logs[0].cpuUsage),
                        )
                      : 0,
                  highestRamUsage: highestStats?.length
                    ? Math.round(highestStats[0].ram)
                    : 0,
                  ramChange:
                    highestStats?.length && logs?.length
                      ? Math.max(
                          0,
                          Math.round(highestStats[0].ram - logs[0].ramUsage),
                        )
                      : 0,
                }}
              />
        ...
        >
          {isEmptyPending ? (
            ...
            <>
              <GraphicRAM
                dataMetrics={chartData}
                totalRam={machine.totalRam || 8}
              />
              <GraphicHardisk
                dataMetrics={chartData}
                totalDisk={machine.totalDisk || 256}
              />
