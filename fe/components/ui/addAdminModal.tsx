...
export default function AddAdminModal({ isOpen, onClose }: AddAdminModalProps) {
  ...
  if (status === "success") {
    return (
      <div
        id="add-admin-success-modal"
        className="fixed inset-0 z-[99999] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-fade-in"
      >
        <div
          id="add-admin-success-card"
          className="bg-white rounded-2xl p-10 max-w-sm w-full shadow-2xl flex flex-col items-center gap-4 animate-scale-up"
        >
          <div
            id="add-admin-success-icon"
            className="w-16 h-16 rounded-full bg-emerald-400 border-[4px] border-emerald-100 flex items-center justify-center mb-2 shadow-sm"
          >
            <svg
              width="32"
              height="32"
          ...
          <h3
            id="add-admin-success-title"
            className="text-xl font-bold text-emerald-500 mb-2"
          >
            Success Added Admin
          </h3>
  ...
  if (status === "error") {
    return (
      <div
        id="add-admin-error-modal"
        className="fixed inset-0 z-99999 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-fade-in"
      >
        <div
          id="add-admin-error-card"
          className="bg-white rounded-2xl p-10 max-w-sm w-full shadow-2xl flex flex-col items-center gap-4 animate-scale-up"
        >
          <div
            id="add-admin-error-icon"
            className="w-16 h-16 rounded-full bg-red-500 border-[4px] border-red-100 flex items-center justify-center mb-2 shadow-sm"
          >
            <svg
              width="32"
              height="32"
          ...
          <div id="add-admin-error-text" className="text-center">
            <h3
              id="add-admin-error-title"
              className="text-xl font-bold text-red-500 mb-2"
            >
              Failed to Add Admin
            </h3>
            <p id="add-admin-error-message" className="text-gray-500 text-sm">
              {serverErrorMsg}
            </p>
          </div>

          <div id="add-admin-error-actions" className="flex w-full gap-3 mt-4">
            <button
              id="add-admin-error-retry"
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div id="add-admin-email-field" className="flex flex-col gap-2">
          <label
            className={`text-sm font-medium ${errorMsg ? "text-pink-500" : "text-gray-600"}`}
          >
            Admin Email*
          </label>
          <input
          ...
          {errorMsg && (
            <p
              id="add-admin-email-error"
              className="text-pink-500 text-xs text-right mt-1"
            >
              {errorMsg}
            </p>
          )}
