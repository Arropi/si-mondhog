...
export default function PeakPerformance({ data }: { data: PeakMetrics }) {
  const peakData = {
    ram: {
      used: parseFloat(
        (((data?.maxRamUsage || 0) / 100) * (data?.maxRam || 0)).toFixed(1),
      ),
    ...
    cpu: {
      used: parseFloat(
        (((data?.maxCpuUsage || 0) / 100) * (data?.maxCpu || 0)).toFixed(1),
      ),
      total: data?.maxCpu || 0,
    ...
    disk: {
      used: parseFloat(
        (((data?.maxDiskUsage || 0) / 100) * (data?.maxDisk || 0)).toFixed(1),
      ),
  ...
  return (
    <div id="dashboard-peak-performance-server">
      <PeakPerformanceClient initialData={peakData} />
    </div>
  );
}
